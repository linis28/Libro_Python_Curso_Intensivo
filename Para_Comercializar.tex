...
\documentclass{book}
\usepackage[spanish]{babel} % Para definir el idioma del documento
\usepackage[utf8]{inputenc} % Para definir la codificación de caracteres
\usepackage[T1]{fontenc}    % Para definir la codificación de fuentes
\usepackage{lipsum}         % Para generar texto de ejemplo

% Configuración de las dimensiones de la página
\usepackage[paperwidth=15cm, paperheight=21cm, inner=2cm, outer=2cm, top=2cm, bottom=2cm]{geometry}
...
\begin{document}
	...
	%\include{PortadaAure} 	
	% \include{PortadaC1}   % La portada original
	\include{Portada_Python} 
	%\input{Python_Cap1} 
	
	%\include{Python_Cap01_error} % {Python_Cap00} 
	\include{Python_Cap01} 
	\include{Python_Cap02} 
	\include{Python_Cap03}
	\include{Python_Cap04}
	\include{Python_Cap05}
	\include{Python_Cap06}
	\include{Python_Cap07}
	%	\tableofcontents
	
	
	%	\chapter{Introducción}
	%	\lipsum[1-2] % Texto de ejemplo
	
	%	\chapter{Capítulo 1}
	%	\lipsum[3-5] % Más texto de ejemplo
...
%\include{Python_Cap02} 
  ...
  % \include{Continuacion_Aurelio}
   %\include{Redes}
                     % Cargamos la portada desde la Carpeta contenido  
  % \include{Introducion17_09_2020}     % Cargamos los capítulos desde la carpeta Contenido.
